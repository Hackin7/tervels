---
title: "Copenhagen — Day 3 (3 June 2026)"
timestamp: 2026-06-03T00:00:00Z
locations:
  - name: "Christiansborg Palace"
    country: "Denmark"
    city: "Copenhagen"
    city_slug: "copenhagen"
    gps: [55.6756505,12.5800406]
    gps_source: openstreetmap
    gps_granularity: building
    gps_confidence: high
    gps_query: "Christiansborg Palace, Copenhagen, Denmark"
  - name: "Jægersborg Dyrehave"
    country: "Denmark"
    city: "Klampenborg"
    city_slug: "klampenborg"
    gps: [55.7908,12.5736]
    gps_source: old-frontmatter
    gps_granularity: area
    gps_confidence: medium
  - name: "Copenhagen market"
    country: "Denmark"
    city: "Copenhagen"
    city_slug: "copenhagen"
    gps: [55.676098,12.568337]
    gps_source: old-frontmatter
    gps_granularity: city
    gps_confidence: low
youtube:
  items:
    - kind: video
      id: "IgUsHcHbuns"
    - kind: video
      id: "Vx8eo-Q7FT4"
    - kind: video
      id: "GPXSwbcyiqM"
  cover: "IgUsHcHbuns"
experiences:
  - kind: trail
    name: "Jægersborg Dyrehave"
  - kind: food
    name: "Denmark Master Chef"
  - kind: food
    name: "Jægersborg Dyrehave cafe"
  - kind: market
    name: "Copenhagen market"
tags: ["exchange-12-dokomi-scandinavia","denmark","copenhagen","klampenborg"]
draft: false
---

<!-- consolidated-from: content_old/posts/2026/exchange-12-dokomi-scandinavia/2026-06-02-tldr-things-did/index.md#denmark-trip-recap -->

## Denmark trip recap

Tldr of things I did in Demark (updates coming soonish) 
1. Eat roast pork
2. See the greatest attraction of denmark
3. See the old castle
4. Eat risotto and Sleep on the floor
5. Road Trip to Adventure Cove and Mont Klint
    - invent maimai🤡
    - play maimai🤡
6. Eat Mala and ice cream and bread
7. See deers & walk 14.5km+
    - see horses
8. See palace and high up
9. Walk 5km to bus station

Didn't leave codex running in the background enough but was fun

---

<!-- consolidated-from: content_old/posts/2026/exchange-12-dokomi-scandinavia/2026-06-03-mala-denmark-master-chef/index.md -->

## Mala from Denmark Master chef (all

Mala from Denmark Master chef (all I can do is wash dishes like the peasant I am)

Needless to say the Mala is amazing

---

<!-- consolidated-from: content_old/posts/2026/exchange-12-dokomi-scandinavia/2026-06-03-unesco-deer-park-near/index.md -->

## UNESCO Deer Park near the dorm

UNESCO Deer Park near the dorm

Horses in the deer park
Also the meal at the Cafe is good stuff, 32sgd 😢 huge burger with roast pork

---

<!-- consolidated-from: content_old/posts/2026/exchange-12-dokomi-scandinavia/2026-06-03-back-around-copenhagen-skimmed/index.md -->

## Back Around Copenhagen Skimmed through the

Back Around Copenhagen
Skimmed through the market, it was OK (but Denmark prices so)

---

<!-- consolidated-from: content_old/posts/2026/exchange-12-dokomi-scandinavia/2026-06-03-christianborg-palace-free-go/index.md -->

## Christianborg Palace Was free to go

Christianborg Palace

Was free to go up and so I did
The view is really not bad.
I really find Denmark's skyline quite unique, you see powerplants, windmills etc.

Other things round the castle

The garden had some really really nice flowers, even though it's not really in season

Statues are OK, tbh the rest is quite standars

The highlight of the Palace is the top view and the garden imo

---

<!-- consolidated-from: content_old/posts/2026/exchange-12-dokomi-scandinavia/2026-06-03-went-cafe-chilled/index.md -->

## Went to a cafe and chilled,

Went to a cafe and chilled, Ate some potato pizza for 12SGD (and shared :) 

Was a decent 3 days in Denmark and Copenhagen, good way to wash off the fomo at Dokomi and Duesseldorf 

Went to a fair bit of places in Denmark and Copenhagen, road trip!!! Monts Klint was amazing suggestion, and Copenhagen itself is very nice, blocky cuboid trees, windmills power plants, modern architecture and very clean. After going to Germany and Eastern Europe, this is really a breath of fresh air

Have to thank the greatest attraction of Denmark for hosting me despite having an exam tmr and flying back to SG in 2 days time LOL
