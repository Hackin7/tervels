---
title: "Stockholm — Day 1 (3 June 2026)"
timestamp: 2026-06-03T00:00:00Z
locations:
  - name: "Vasa Museum"
    country: "Sweden"
    city: "Stockholm"
    city_slug: "stockholm"
    gps: [59.3280594,18.0913656]
    gps_source: openstreetmap
    gps_granularity: building
    gps_confidence: high
    gps_query: "Vasa Museum, Stockholm, Sweden"
  - name: "HEY STHLM"
    country: "Sweden"
    city: "Stockholm"
    city_slug: "stockholm"
    gps: [59.3424511,18.0523651]
    gps_source: openstreetmap
    gps_granularity: venue
    gps_confidence: high
    gps_query: "HEY STHLM, Stockholm, Sweden"
youtube:
  items:
    - kind: video
      id: "xWXeTyPuhE0"
  cover: "xWXeTyPuhE0"
experiences:
  - kind: museum
    name: "Vasa Museum"
  - kind: event
    slug: "midnight-sun-ctf-2026"
tags: ["exchange-12-dokomi-scandinavia","sweden","stockholm","midnight-sun-ctf-2026"]
draft: false
---

<!-- consolidated-from: content_old/posts/2026/exchange-12-dokomi-scandinavia/2026-06-04-stockholm-crashed-midnight-sun/index.md -->

## Stockholm Crashed the Midnight Sun Team

Stockholm
Crashed the Midnight Sun Team to follow around lol

Decent view, tho I feel it's more similar to the rest of Europe

---

<!-- consolidated-from: content_old/posts/2026/exchange-12-dokomi-scandinavia/2026-06-04-place-some-good-stuff/index.md -->

## This place is some good stuff

This place is some good stuff
I think the Swedish toast that I got is actually quite nice, cold prawn seafood stuff

---

<!-- consolidated-from: content_old/posts/2026/exchange-12-dokomi-scandinavia/2026-06-04-vasa-museum-really-cool/index.md -->

## Vasa museum Really cool ship I

Vasa museum
Really cool ship

I find it a very interesting way to make a museum, they have 1 central topic, and explain everything about it

Frankly the side things aren't really too unique but you see the huge ship and that really makes you think

Though it was quite funny I left without knowing why it sunk (I was so tired I slept through that film)

---

<!-- consolidated-from: content_old/posts/2026/exchange-12-dokomi-scandinavia/2026-06-04-dinner-crashed-midnight/index.md -->

## Dinner crashed midnight sun people

Dinner
- crashed midnight sun people
- the place was meh, but there was a LOT
- like LakeCTF vibes, 1 team 1 table, though we met with Linz which was nice
- not really as much vibes to network as with Insomnihack, and then Lake (Lake had various aperos to network which was really nice) 

Supermarket is ok
I find it interesting
- the croissants are more expensive than most of Europe I feel
- dedicated salad buffet style thingy

---

<!-- consolidated-from: content_old/posts/2026/exchange-12-dokomi-scandinavia/2026-06-04-only-took-me-3/index.md -->

## Only took me 3.5months in Europe

Only took me 3.5months in Europe to finally go to a real arcade and play maimai🤡

Anyway it was freaking satisfying to finally do that
